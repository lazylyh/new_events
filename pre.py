...
@hydra.main(config_path='config', config_name='pre', version_base='1.3')
def main(config: DictConfig):
    ...
    # ---------------------
    logger = CSVLogger(save_dir='./predict_logs')
    ...
    module = RAFTSplineModule.load_from_checkpoint(str(ckpt_path), 
                                                   map_location='cuda:0', 
                                                   **{'config': config})
    ...
    with torch.inference_mode():
        trainer.predict(model=module, datamodule=data_module,
